...
namespace gen_riccati
{
    ...
    {
        public:
        COCP(const int K, const NumericVector &nu, const NumericVector &nx, const NumericVector &ng) : K(K), nu(nu), nx(nx), ng(ng), RSQrqt(nu + nx + 1, nu + nx, K), BAbt(nx + nu+ 1, rotate(nx, 1), K-1), Ggt(ng, nx, K)
        {
        }
